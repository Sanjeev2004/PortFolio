...
const MiniTerminal = () => {
  ...
  const handleCommand = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      addLine(`guest@sanjeev:~$ ${input}`, 'input');
      ...
      switch (cmd) {
        case 'help':
          ...
          break;
        case 'whoami':
          addLine('Sanjeev Kumar | B.Tech CSE @ Delhi Technological University');
          addLine('AI/ML Engineer · Software Developer');
          break;
        case 'skills':
          addLine('► Python, C++, SQL, JavaScript, TypeScript');
          addLine('► TensorFlow, PyTorch, LangGraph, FastAPI, RAG');
          addLine('► LLMs, Multi-Agent Systems, NLP, Deep Learning');
          break;
        case 'projects':
          addLine('1. Soch Samajh AI — Multi-Agent Query Router');
          addLine('2. Mental Health Emotion Detection — DistilBERT');
          addLine('3. Speech Emotion Recognition — CNN + LSTM');
          addLine('4. Document QA System — RAG + FAISS');
          break;
        case 'game':
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-black border-4 border-black rounded-2xl shadow-neo overflow-hidden flex flex-col font-mono text-sm sm:text-base">
      <div className="bg-gray-200 border-b-4 border-black px-4 py-2 flex items-center justify-between">
        ...
        <span className="font-bold text-black text-xs tracking-widest">root@sanjeev:~</span>
        <div className="w-10"></div>
      </div>
      <div className="p-4 h-64 overflow-y-auto bg-black text-custom-blue custom-scrollbar">
        {history.map((line, index) => (
          <div key={index} className={`mb-1 ${line.type === 'input' ? 'text-white' : 'text-custom-blue'}`}>
            {line.text}
          </div>
        ))}
        <div className="flex items-center mt-2">
          <span className="text-custom-green mr-2">guest@sanjeev:~$</span>
          <input
            type="text"
            value={input}
