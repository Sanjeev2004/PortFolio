    ...
    <>
      <nav className="fixed top-4 left-4 right-4 z-50 bg-custom-purple border-4 border-black rounded-full px-4 py-3 shadow-neo flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="hidden lg:flex gap-3 font-bold text-sm items-center">
          ...
          <a
            href="/Sanjeev_Resume_v9.pdf"
            target="_blank"
            rel="noopener"
            className="px-4 py-2 bg-custom-yellow text-black rounded-full border-black border-b-4 border-r-4 border-2
                       hover:border-b-2 hover:border-r-2 active:translate-y-1 transition-all cursor-pointer font-bold text-sm"
          >
            RESUME
          </a>
          <button
      ...
      {isMenuOpen && (
        <div className="fixed top-24 left-4 right-4 z-40 bg-custom-purple border-4 border-black rounded-3xl p-4 flex flex-col gap-3 shadow-neo lg:hidden max-w-7xl mx-auto">
          {navLinks.map((link) => (
            <button
              key={link.name}
              onClick={() => scrollToSection(link.id)}
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            className="bg-custom-yellow border-2 border-black border-b-4 border-r-4 p-3 rounded-xl font-bold active:border-b-2 active:border-r-2 active:translate-y-1 text-left transition-all"
          >
            RESUME
          </a>
          <button
